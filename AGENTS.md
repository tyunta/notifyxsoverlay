NOTE: This file is UTF-8 (no BOM). If it looks garbled in Shift_JIS, reopen as UTF-8.
全テキストはUTF-8（BOMなし）で読み書きする。S-JIS禁止。

# NotifyXSOverlay 憲法

## 目的
- Windows通知をXSOverlayへ中継し、VR内で確実に視認できるようにする
...
### ドキュメント
- READMEは初見ユーザー向けの導線を最優先にする
- README.devは開発者向けの詳細に限定する
- セットアップ手順、前提条件、必要な権限はREADMEで明示する

### テスト
- 実行: `uv run --extra test --with pytest pytest`
- カバレッジ: `uv run --extra test --with pytest-cov pytest --cov=src/notifyxsoverlay --cov-report=term-missing:skip-covered`
- 仕様/挙動/設定に関わる変更はテスト必須
- ドキュメントのみの変更は任意

### コミット
- 変更は小さく分け、必要なら `git add -p` で行単位にステージしてからコミットする
- コミットメッセージは変更内容が分かる簡潔な文言にする

### レビュー
- 仕様/READMEとの整合性が取れているか
- 設定ファイルの互換性が壊れていないか
- フィルタ評価順が意図通りか
- 失敗時にログが残り、静かに壊れないか
- SteamVR/XSOverlayの前提条件が文書化されているか
